		...
		foreach ($r->input('col_name_convention') as $conv) {
			...
			if (preg_match('/(\d+)\+(\d+)|,/i', $conv)) {
				...
				if (!empty($final_pluckName) && request()->has('forginkeyto' . $i2) && !empty(request('dt_show_column')) && in_array($pre_conv[0], request('dt_show_column'))) {
					...
					$cols .= '                 \'name\'=>\'' . substr($pre_conv[0], 0, -3) . '.' . $final_pluckName . '' . '\',' . "\n";
					$cols .= '                 \'data\'=>\'' . substr($pre_conv[0], 0, -3) . '.' . $final_pluckName . '\',' . "\n";
					$cols .= '                 \'title\'=>trans(\'{lang}.' . substr($pre_conv[0], 0, -3) . '\'),' . "\n";
					$cols .= '		    ],' . "\n";
				} elseif (!request()->has('forginkeyto' . $i2) && !empty(request('dt_show_column')) && in_array($pre_conv[0], request('dt_show_column'))) {
					$cols .= '				[' . "\n";
					$cols .= '                 \'name\'=>\'' . self::convention_name(request('model_name')) . '.' . substr($pre_conv[0], 0, -3) . '\',' . "\n";
					$cols .= '                 \'data\'=>\'' . substr($pre_conv[0], 0, -3) . '\',' . "\n";
		...
		foreach ($r->input('col_name_convention') as $conv) {
			if (preg_match('/(\d+)\+(\d+)|,/i', $conv)) {
				...
				if ($r->has('forginkeyto' . $i2)) {
					// Edid by Naser for Delete _id from name of relation
					$WithRelation .= "'".substr($pre_conv[0], 0, -3)."', ";
				}
			}
			$i2++;
		}
