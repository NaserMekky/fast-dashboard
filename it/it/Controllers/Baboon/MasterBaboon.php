...
class MasterBaboon extends Controller
{
	...
	public static function makeModel($namespace, $classname)
	{
		...
		if (request()->has('linkatmodel')) {
			...
			foreach (request('linkatmodel') as $linkat) {
				// Add by Naser Mekky To Rename relation
				$name = explode('_', request('schema_name')[$i]);
				...
				$model .= '
					...
					* @param void
					* @return object data
					* Edite Name Of Function By Naser Mekky 
					* To Be Stander Of Laravel
					*/
	...
	public static function check_path($path)
	{
		...
		if (!is_dir(base_path($checkpath))) {
			mkdir(base_path($checkpath), 0777);
		}
		...
		foreach ($path as $p) {
			...
			if (!is_dir(base_path($full_path)) || !file_exists(base_path($full_path))) {
				...
				mkdir(base_path($full_path), 0777);
			}
		}
		return true;
	}
	...
	public static function inputsCreate($r)
	{
		...
		if ($r->has('col_type') and $r->has('col_name')) {
			...
			foreach ($r->input('col_name') as $col_name) {
				...
				} elseif (!empty($r->input('col_type')[$i]) and 'number' == $r->input('col_type')[$i]) {
					$input .= BaboonCreate::number($data);
	...
	public static function actions($r)
	{
		...
		$edit = '
		<div class="btn-group">
			<button type="button" class="btn btn-success btn-flat dropdown-toggle" data-toggle="dropdown"><i class="fa fa-wrench"></i> {{ trans(\'{file_lang}.actions\') }}
			<span class="caret"></span>
			<span class="sr-only"></span>
			</button>
			<div class="dropdown-menu" role="menu">
