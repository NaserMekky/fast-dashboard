...
class Home extends Controller
{
	...
	public function index_post()
	{
		...
		// 	return response(['status' => false, 'message' => $e->getMessage()." <br> Error Code: " .$e->getCode()],400);
		// }
	...
	public function makeControllerAndApi()
	{
		if (request()->has('make_controller')) {
			$controller = Baboon::makeController(
				request(),
				request('controller_namespace'),
		...
		$controllerApi = Baboon::makeControllerApi(
			request(),
			request('controller_namespace'),
			request('model_namespace') . '\\' . request('model_name'),
			request('controller_name')
