@push('baboon_js')
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $(document).on('click', '.link_ajax', function() {
                ...
                if ($(this).is(':checked')) {
                    var select_list_to_ajax = '<select name="select_ajax_link' + to +
                        '" class="form-control">';
                    $('input[name="col_name_convention[]"]').each(function() {
                        var vselect = $(this).val();
                        select_list_to_ajax += '<option value="' + vselect + '">' + vselect +
                            '</option>';
                    });
            ...
            var x =
            {{ !empty($module_data) && $module_data->count_inputs > 0 ? $module_data->count_inputs - 1 : 0 }}; //initlal text box count
            ...
            var x2 =
                {{ !empty($module_data) && $module_data->relation_count > 0 ? $module_data->relation_count - 1 : 0 }}; //initlal text box count
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $.each(fa_lists, function(k, v) {
                icons_list +=
                    '<div class="col-md-2" style="margin-bottom:5px"><center><a href="#" class="getfa" title="' +
                    v + '" fa="' + v + '" ><i class="' + v + ' fa-2x" aria-hidden="true"></i></a><p>' + v +
                    '</p></center></div>';
            });
            ...
            if (search_list != '') {
                ...
                for (var i = 0; i < fa_lists.length; i++) {
                    if (fa_lists[i].match(search_list)) {
                        icons_list +=
                            '<div class="col-md-2" style="margin-bottom:5px"><center><a href="#" class="getfa" fa="' +
                            fa_lists[i] + '" ><i class="' + fa_lists[i] + ' fa-2x" title="' + fa_lists[i] +
                            '" aria-hidden="true"></i></a><p>' + fa_lists[i] + '</p></center></div>';
                    }
                }
                $('.bodyfalist').append(icons_list);
                ...
                for (i = 0; i < fa_lists.length; i++) {
                    $('.bodyfalist').append(
                        '<div class="col-md-2" style="margin-bottom:5px"><center><a href="#" class="getfa" fa="' +
                        fa_lists[i] + '" ><i class="' + fa_lists[i] + ' fa-2x"></i></a><p>' + fa_lists[i] +
                        '</p></center></div>');
                }
            }
            if (p_title != '') {
        ...
        $(document).on('click', 'a.addcnamespace', function(e) {
            ...
            $('.controller_namespace').append('<option value="App\\Http\\Controllers\\' + namespace +
                '" selected>App\\Http\\Controllers\\' + namespace + '</option>');
            $('.preview_c_namespace').text('');
            $('.c_namespace').val('');
            $.ajax({
    ...
    <script type="text/javascript">
     ...
     `;
                table_cols_footer += `<th class="datatable_checkbox ` + hideOrShowRow(
                    'datatable_checkbox') + `"></th>`;
                // datatable_checkbox //
     ...
     `;
                table_cols_footer += `<th class="datatable_record_id ` + hideOrShowRow(
                    'datatable_record_id') + `">
            ...
            ` + col_name_datatable[i] + `
             <input type="checkbox" {{ !empty(request('module')) ? '' : 'checked' }} class="dt_checkbox" name="dt_show_column[]" value="` +
                        conv_col_checkbox[0] + `" />
            </label>
        </th>
        `;
                ...
                setTimeout(function() {
                    ...
                    for (i = 0; i < col_name_datatable.length; i++) {
                        ...
                        table_cols += `<th class="` + conv_col[0] + ` ` + hideOrShowRow(conv_col[
                            0]) + `">` + col_name_datatable[i] + `</th>`;

                        if (conv_col[1] !== '' && conv_col[1] !== undefined) {
                            table_cols_footer += `<th class="` + conv_col[0] + ` ` + hideOrShowRow(
                                conv_col[0]) + `">
     ...
     `;
                    table_cols_footer += `<th class="datatable_created_at ` + hideOrShowRow(
                        'datatable_created_at') + `"></th>`;
                    // datatable_created_at //
     ...
     `;
                    table_cols_footer += `<th class="datatable_updated_at ` + hideOrShowRow(
                        'datatable_updated_at') + `"></th>`;
                    // datatable_updated_at //
     ...
     `;
                    table_cols_footer += `<th class="datatable_action ` + hideOrShowRow(
                        'datatable_action') + `"></th>`;
                    // datatable_action //
